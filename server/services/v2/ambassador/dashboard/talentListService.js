...
class TalentListService {
    ...
    static async getInvitedTalents (req, user, local) {
        ...
        TalentListService.commonTalentListCondition(user._id, statusSearch);
        ...
        aggregateParams.push({
            $project: {
                ...
                'status': {
                    $switch: {
                        branches: [
                            TalentListService.invitedStatus( 'Invited'),
                            TalentListService.registeredStatus('Registered'),
                            TalentListService.hiredStatus('Hired'),
                            TalentListService.verifiedStatus('Verified'),
                            TalentListService.activeStatus('Active')
                        ],
                        default: '-'
                    }
                }
        ...
        TalentListService.commonTalentListCondition2(req.query, statusSearch2, local);
        ...
        if (req.query.q) {
            ...
            aggregateParams.push({
                $match: {
                    $or: [
                        { 'referee.firstName': { $regex: query } },
                        { 'referee.lastName': { $regex: query } },
                        { email: { $regex: query } }
                    ]
                }
            });
    ...
    static activeStatus (result) {
        return {
            case: {
                $and: [
                    TalentListService.IsActiveCondition(),
                    TalentListService.signupStepCondition()
                ]
            },
            then: result
        };
        ...
        return {
            case: {
                $and: [
                    TalentListService.IsActiveCondition(),
                    TalentListService.signupStepCondition(),
                    TalentListService.verifiedCondition()
                ]
            },
            then: result
        };
        ...
        return {
            case: {
                $and: [
                    TalentListService.IsActiveCondition(),
                    TalentListService.signupStepCondition(),
                    TalentListService.hiredCondition()
                ]
            },
            then: result
        };
