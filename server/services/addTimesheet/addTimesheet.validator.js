...
const { TIMESHEET: { VALID_VALUES, HOURS, MINUTES } } = require('../../util/constants');
...
class TimeSheetValidator extends validation {
    ...
    checkHours(hours) {
        const { MIN, MAX } = HOURS.LENGTH;
        if (MIN > hours || MAX < hours) {
    ...
    convertHoursToDay(hours) {
        let day = 0;
        if (hours && hours > 0 && hours < 4) {
            day = 0.5;
        } else if (hours && hours > 4) {
            day = 1;
        }
        return day;
    }
    ...
    checkMinutes(minutes) {
        const { MIN, MAX } = MINUTES.LENGTH;
        if (MIN > minutes || MAX < minutes) {
            throw new CodeMonkError(this.__('FIELD_LENGTH', { FIELD: 'Minutes', MIN, MAX }), 400);
        }
        return minutes;
    }
    ...
    checkWeek() {
        ...
        return this.body.week.map((obj) => {
            ...
            return {
                date: date.toDate(),
                hours: this.checkHours(obj.hours),
                minutes: this.checkMinutes(obj.minutes),
                value: this.convertHoursToDay(obj.hours)
            };
        });
    }
}
