const chai = require('chai');
const chaiHttp = require('chai-http');
const expect = chai.expect;
const assert = chai.assert;
const request = require('supertest');
const TestCase = require('./testcaseAgencyDeleteTalent');
chai.use(chaiHttp);
const jwt = require('jsonwebtoken');
const tokenOptionalInfo = {
    algorithm: 'HS256',
    expiresIn: 86400
};
// Agency User
const agencyUser = {
    id: '5f475a9ef25e122eb21d68a8',
    email: 'agency@mailinator.com'
};
const requestPayload = {
    token: jwt.sign(agencyUser, process.env.JWT_SECRET, tokenOptionalInfo)
};

// Agency User
const agencyOnboardUser = {
    id: '5f4754eb3fc8842306a8220d',
    email: 'agencystart@mailinator.com'
};
const agencyOnBoardPayload = {
    token: jwt.sign(agencyOnboardUser, process.env.JWT_SECRET, tokenOptionalInfo)
};
describe('Agency talent delete', () => {
    try {
        TestCase.deleteTalent.forEach((data) => {
            it(data.it, (done) => {
                request(process.env.BASE_URL)
                    .delete('/agency/talent')
                    .set({ Authorization: requestPayload.token })
                    .send(data.options)
                    .end((err, res) => {
                        expect(res.body.status).to.be.status;
                        assert.equal(res.body.status, data.status);
                        assert.equal(res.statusCode, 400);
                        done();
                    });
            });
        });

        it('As a agency I can delete my talent', async () => {
            const talents = {
                email: 'talentnew2@mailinator.com'
            };
            const res = await request(process.env.BASE_URL)
                .delete('/agency/talent')
                .send(talents)
                .set({ Authorization: requestPayload.token });

            expect(res.body.status).to.be.status;
            assert.equal(res.statusCode, 200);
        });

        it('As a agency I can delete my talent from so I can remove my association.', async () => {
            const talents = {
                email: 'agencyonboardtalent1@mailinator.com'
            };
            const res = await request(process.env.BASE_URL)
                .delete('/agency/talent')
                .send(talents)
                .set({ Authorization: agencyOnBoardPayload.token });

            expect(res.body.status).to.be.status;
            assert.equal(res.statusCode, 200);
        });
    } catch (exception) {
        CONSOLE_LOGGER.error(exception);
    }
});

