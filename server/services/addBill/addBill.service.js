...
class AddBillService {
        ...
        const bills = await TimeSheet.aggregate([
            ...
            },
            {
                $project: {
                    ...
                    email:AddBillService.emailProjection(),
    ...
    static emailProjection() {
        return {
            $cond: [{ $eq: [CONSTANTS.TALENT_REGISTER_TYPE.AGENCY, { $arrayElemAt: ["$talent.registerType", 0] }] },
            { $arrayElemAt: ["$agencyuser.email", 0] },
    ...
    static async prepareInvoiceObj(bill) {
        ...
        const week = JSON.parse(JSON.stringify(bill.week))
        for (const w of week) {
        ...
        return {
            ...
            logoImage : logoImage,
            email:this.combineTitleAndValue('Email', bill.email),
        }
    }
