module.exports = {
  async up(db, client) {
    ...
    const certificationsArr = certifications.map((i) => {
      return {
        name: i, active: 1,
        createdAt: new Date(),
        updatedAt: new Date()
    ...
    // Bulk insert
    var bulk = db.collection('certifications').initializeOrderedBulkOp();
    for (let i = 0; i < certificationsArr.length; i++) {
      bulk.insert(certificationsArr[i]);
    }
    bulk.execute();
  },
